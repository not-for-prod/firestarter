---
name: error-handling
description: Classify, wrap, and propagate Go errors consistently with `proterror`. Use when the user asks to wrap errors, add structured error handling, classify repository/gateway/application errors, choose `proterror` types, avoid double-wrapping, or align error propagation with tracing via `span.Err(...)`.
---
...
```go
import "github.com/not-for-prod/proterror"
```
