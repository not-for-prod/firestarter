...
description: Validate request, response, and DTO fields at their first occurrence in code when adding or changing Go backend data shapes across proto contracts, handlers, services, repositories, gateways, or converters. Prefer `buf.validate` annotations in `.proto` contracts plus `grpcmw.ProtoValidate(validator)` middleware for protobuf fields, and add those proto validations when fields first appear there. Use this to keep field-shape validation owned once, avoid repeated lower-layer checks, and centralize unavoidable code validation in one helper.
---
...
Apply a strict field validation ownership rule in Go backend code:

- validate each request, response, or DTO field at its first occurrence in code
- for gRPC protobuf contracts, the `.proto` field is the first occurrence
...
- If a field is first introduced in an application, service, repository, gateway, or converter request struct, that layer owns the first validation for that field.
