...
description: Apply Firestarter's method-per-file convention as a companion to repository or gateway work. Use when adding or refactoring public adapter methods so method-owned DTOs, constructors, conversion helpers, and the implementation are ordered consistently in one focused file; do not use it for domain entity or application service methods.
---
...
Prefer existing package conventions when they are already clear, but use this convention for new adapter files and focused adapter refactors.
