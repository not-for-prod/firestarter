---
name: project-layout
description: Answer direct Firestarter file-placement, package-boundary, navigation, and dependency-direction questions. Use when deciding where code belongs under `cmd`, `config`, `api`, `internal`, or `tools`, or when moving files between layers; use `engineering-judgment` first only when the underlying ownership or design is genuinely uncertain.
---
...
- use `domain-entity` for aggregates, value objects, and domain events
- use `application-layer` for commands, queries, and transaction ownership
- use `delivery-layer` for API handlers, workers, and transport mapping
...
- `api` contains protobuf contracts and API schemas under the service subtree created or renamed by `make init`
...
The template keeps its executable entrypoint and Fx modules directly under `cmd/` and runs with `go run ./cmd`. Add files such as `main.go`, `public_service_module.go`, or feature-specific module files to that package when extending the single service.

Introduce `cmd/<binary>` only when the project deliberately adds multiple executables and updates its run, build, and wiring commands accordingly. Do not move the existing entrypoint merely to match a generic Go layout.
...
Use the existing service subtree under `api/` for protobuf and public contract files. The bootstrap command renames the initial `api/firestarter` subtree, so discover the current package path instead of hard-coding the template name.
...
- commands selected by delivery events, worker payloads, cron triggers, and API requests
- batch commands that load aggregate sets, call aggregate transition methods, persist changed state, and publish domain events

This layer should express the use case, not transport concerns or low-level client details. When a state-changing event/request arrives, delivery chooses the application command; the command owns aggregate loading, domain transition, persistence, and event publication.
...
- grouping work into batch service requests
- deleting or acknowledging processed items

Do not put business state transitions here when they belong in aggregates or application services. Workers must not load aggregates for lifecycle updates, call aggregate transition methods, persist business state, or create lifecycle domain events. If a worker receives many items for the same command, call a batch application command instead of looping over a single-item command when repository batch operations are practical.
