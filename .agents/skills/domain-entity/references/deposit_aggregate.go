...
func RehydrateDeposit(snapshot Deposit) (*Deposit, error) {
	switch {
	case snapshot.ID.IsZero():
		return nil, ErrDepositIDRequired
	case snapshot.PaymentID.IsZero():
		return nil, ErrPaymentIDRequired
	case snapshot.RequisiteID.IsZero():
		return nil, ErrRequisiteIDRequired
	case snapshot.Amount.IsNegative():
		return nil, ErrNegativeAmount
	case snapshot.Fee.IsNegative():
		return nil, ErrNegativeFee
	case snapshot.CreatedAt.IsZero():
		return nil, ErrCreatedAtRequired
	case !snapshot.State.IsValid():
...
func (s DepositState) IsValid() bool {
	switch s {
	case DepositStatePending,
